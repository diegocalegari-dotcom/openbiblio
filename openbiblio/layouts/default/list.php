...
class Layout_list
{
    function render($rpt)
    {
        ...
        # We have 10in, distribute it weighted by width
        $unit = (10 - $totalspacing) / $total;

        $lay = new Lay('letter', 'landscape');
        ...
        while ($row = $rpt->each()) {
            ...
            foreach ($cols as $col) {
                ...
                if ($col['name'] == 'author') {
                    ...
                    if (strpos($trimmed_text, ',') === false) {
                        $parts = explode(' ', $trimmed_text, 2);
                        if (count($parts) > 1) {
