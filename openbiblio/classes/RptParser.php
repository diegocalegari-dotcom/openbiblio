...
class RptParser
{
    ...
    function p_param_decls()
    {
        ...
        while (1) {
            if ($this->lat[0] == 'order_by') {
                ...
                if (is_a($params, 'ObibError')) {
                    return $params;
                }
                $items = $this->p_items();
                if (is_a($items, 'ObibError')) {
                    return $items;
                }
                $result = $this->p_end();
                if (is_a($result, 'ObibError')) {
    ...
    function p_sql_form()
    {
        ...
        if (is_a($exprs, 'ObibError')) {
            return $exprs;
        }
        $subs = $this->p_subselects();
        if (is_a($subs, 'ObibError')) {
            return $subs;
        }
        $result = $this->p_end();
        if (is_a($result, 'ObibError')) {
    ...
    function p_sql_expr()
    {
        switch ($this->lat[0]) {
            ...
            case 'if_set':
                ...
                if (is_a($then, 'ObibError')) {
                    return $then;
                }
                $else = $this->p_else_part();
                if (is_a($else, 'ObibError')) {
                    return $else;
                }
